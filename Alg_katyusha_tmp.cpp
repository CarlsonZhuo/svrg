...
mxArray* Katyusha_dense(int nlhs, const mxArray *prhs[]) {
    ...
    double Lmax     = mxGetScalar(prhs[8]);
    double tau1     = 1 + lambda*stepSize;
    double tau2     = Lmax + lambda;
    double tau3     = Lmax/tau2;
    ...
    for (k = 0; k < iters; k++)
    {
        ...
        for (i = 0; i < m[k]; i++) {
            ...
            for (long j = 0; j < d; j++) {
                double tmp_g;
                tmp_g = gold[j] + Xt[j+d*idx] *
                    (sigmoid - sigmoidold) + lambda * (w[j] - wold[j]);
                zm[j] = (1/tau1) * zm[j] - (stepSize/tau1) * tmp_g;
                ym[j] = tau3 * w[j] - (1/tau2) * tmp_g;
                ww[j] += ym[j] * pow(tau1, i);
            }
