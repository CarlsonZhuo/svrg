...
/*
    USAGE:
    hist = ASVRG(w, Xt, y, lambda, stepsize, iVals, m);
    ==================================================================
    INPUT PARAMETERS:
...
mxArray* ASVRG_dense(int nlhs, const mxArray *prhs[]) {
    ...
    double *x = new double[d]; for(k=0;k<d;k++){x[k]=0;}
    ...
    for (k = 0; k < iters; k++)
    {
        ...
        for (i = 0; i < d; i++) { w[i] = wold[i]; x[i] = wold[i];}
        ...
        for (i = 0; i < m[k]; i++) {
            ...
            update_test_point_dense_ASVRG(Xt + d*idx, w, wold, gold, 
                sigmoid, sigmoidold, d, stepSize, lambda, x, tau);
...
mxArray* ASVRG_sparse(int nlhs, const mxArray *prhs[]) {
    ...
    for (k = 0; k < iters; k++)
    {
        ...
        for (j = 0; j < d; j++) { wold[j] = w[j]; x[j] = wold[j]; last_seen[j] = 0; }
        ...
        for (i = 0; i < m[k]; i++) {
            ...
            lazy_update_ASVRG(w, wold, gold, last_seen, stepSize, lambda, i, ir, jc + idx, x, tau);
            ...
            // Update the test point
            update_test_point_sparse_ASVRG(Xt + jc[idx], w, sigmoid,
                sigmoidold, jc[idx + 1] - jc[idx], stepSize, ir + jc[idx], x, tau);
        }

        // Update the rest of lazy_updates
        finish_lazy_updates_ASVRG(w, wold, gold, last_seen, stepSize, lambda, m[k], d, x, tau);
    }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    if (mxIsSparse(prhs[1])) {
        plhs[0] = ASVRG_sparse(nlhs, prhs);
    } else {
        plhs[0] = ASVRG_dense(nlhs, prhs);
    }
}
