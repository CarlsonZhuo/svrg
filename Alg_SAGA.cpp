...
mxArray* SAG_dense(int nlhs, const mxArray *prhs[]) {
    ...
    for (k = 0; k < iters; k++)
    {
        ...
        for (i = 0; i < d; i++) {
            double grad_diff = Xt[d*idx + i] * (sigmoid - sigmoidold[idx]);
            w[i] -= stepSize * (grad_diff + g[i] + lambda * w[i]);
        }
...
mxArray* SAG_sparse(int nlhs, const mxArray *prhs[]) {
    ...
    for (k = 0; k < iters; k++)
    {
        ...
        // Update Test Point SAGA
        double tmp_d = jc[idx + 1] - jc[idx];
        mwIndex *tmp_ir = ir + jc[idx];
        for (long j = 0; j < tmp_d; j++) {
            double grad_diff = Xt[j+jc[idx]]*(sigmoid - sigmoidold[idx]);
            double reg = lambda * w[tmp_ir[j]];
            w[tmp_ir[j]] -= stepSize * (k+1 - last_seen[tmp_ir[j]]) * (g[tmp_ir[j]] + grad_diff + reg);
            last_seen[tmp_ir[j]] = k;
        }
